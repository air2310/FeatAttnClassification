...
function h = rm_raincloud_series(data, colours,  seriesnumber, cmapuse, plot_top_to_bottom, density_type, bandwidth)
...
% switch seriesnumber
%     case 1
%         ks_offsets = ks_offsets + spacing/8; %/8
%     case 2
%         ks_offsets = ks_offsets - spacing/8;
% end
spacing     = 1; %0.8
disp(spacing)
ks_offsets  = [0:n_plots_per_series-1] .* spacing;
switch seriesnumber
    case 1
        ks_offsets = ks_offsets + 0.22; %spacing/8; %/8
    case 2
        ks_offsets = ks_offsets - 0.22;%spacing/8;
end
% spacing     = 8; %0.8
% disp(spacing)
% ks_offsets  = [0:n_plots_per_series-1] .* spacing;
% switch seriesnumber
%     case 1
%         ks_offsets = ks_offsets + 1; %spacing/8; %/8
%     case 2
%         ks_offsets = ks_offsets - 1;%spacing/8;
% end
